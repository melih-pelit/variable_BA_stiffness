...
% Copied from C:\Matlab Workspace\Publication Results\ACC2022 Results\20211004_main_5Link_2phase controller
...
dx_CoM_des = des_traj(6);
dy_CoM_des = des_traj(7);
dx_sw_ft_des = des_traj(8);
dy_sw_ft_des = des_traj(9);
dtorso_angle_des = des_traj(10);

x_CoM_des = des_traj(1);
y_CoM_des = des_traj(2);
x_sw_ft_des = des_traj(3);
y_sw_ft_des = des_traj(4);
torso_angle_des = des_traj(5);
...
F_dX = @(dX) [
        ...
        dtorso_angle_des - (dX(1) + dX(2) + dX(5))];

dX_des = fsolve(F_dX,[X_curr(6),X_curr(7),X_curr(8),X_curr(9),X_curr(10)], optimoptions('fsolve','Algorithm', 'Levenberg-Marquardt'));

des_th = [X_des(1); X_des(2); X_des(3); X_des(4); X_des(5); dX_des(1); dX_des(2); dX_des(3); dX_des(4); dX_des(5)];
time_elapsed = toc;
end