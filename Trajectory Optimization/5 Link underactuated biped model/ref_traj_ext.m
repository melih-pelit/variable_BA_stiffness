...
d_obs = mean(sol.parameters.d_obs.value); % [m] distance of the obstacle from the stance foot
w_obs = mean(sol.parameters.w_obs.value);
h_obs = mean(sol.parameters.h_obs.value);
...
ocl_traj.x_sw = x_sw';
ocl_traj.y_sw = y_sw';
ocl_traj.dx_sw = dx_sw';
ocl_traj.dy_sw = dy_sw';

ocl_traj.trunk = simout(:,1) + simout(:,2) + simout(:,5);
ocl_traj.dtrunk = simout(:,6) + simout(:,7) + simout(:,10);

ocl_traj.simout = simout;

ocl_traj.input_Torques = u;
ocl_traj.inputTorques_ext = u_ext;
